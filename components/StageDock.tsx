...
const stages = [
  { href: "/", roman: "I", label: "Test wiedzy" },
...
export default function StageDock() {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-4 z-50 px-3" aria-label="Etapy egzaminu oficerskiego">
      <div className="mx-auto flex w-fit max-w-full gap-1 overflow-x-auto rounded-2xl border border-neutral-700/80 bg-neutral-950/90 p-1.5 shadow-2xl shadow-black/30 backdrop-blur-xl">
        {stages.map((stage) => {
          const active = stage.href === "/" ? pathname === "/" : pathname.startsWith(stage.href);
          return (
            <Link
              key={stage.href}
              href={stage.href}
